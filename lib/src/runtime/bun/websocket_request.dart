...
final class BunAcceptedWebSocketUpgrade {
  const BunAcceptedWebSocketUpgrade({required this.handler, this.protocol});

  final WebSocketHandler handler;
  final String? protocol;
}

final class BunWebSocketRequest implements WebSocketRequest {
  BunWebSocketRequest(this._request)
    : _requestedProtocols = _parseRequestedProtocols(_request);

  final web.Request _request;
  final List<String> _requestedProtocols;
  BunAcceptedWebSocketUpgrade? _acceptedUpgrade;
  ...
  Response accept(WebSocketHandler handler, {String? protocol}) {
    ...
    _acceptedUpgrade = BunAcceptedWebSocketUpgrade(
      handler: handler,
      protocol: protocol,
    );
