...
function hasTlsFromEnvironment() {
  const cert = Deno.env.get('OSRV_TLS_CERT') ?? Deno.env.get('TLS_CERT');
  const key = Deno.env.get('OSRV_TLS_KEY') ?? Deno.env.get('TLS_KEY');
  return typeof cert === 'string' && cert.length > 0 && typeof key === 'string' && key.length > 0;
}

const presetProtocol = String(Deno.env.get('OSRV_PROTOCOL') ?? 'http').toLowerCase();
