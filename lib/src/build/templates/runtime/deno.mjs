...
function readRequestedHttp2(options) {
  return parseBoolean(
    options.http2 ??
      Deno.env.get('OSRV_DENO_HTTP2') ??
      Deno.env.get('OSRV_HTTP2'),
  );
}
...
export async function serve(options = {}) {
  ...
  const requestedHttp2 = readRequestedHttp2(options);
  if (requestedHttp2 === true && protocol !== 'https') {
    console.warn(
      '[osrv/deno] HTTP/2 requested without HTTPS/TLS, falling back to HTTP/1.1.',
    );
  }
  if (requestedHttp2 === false && protocol === 'https') {
    console.warn(
      '[osrv/deno] Deno.serve TLS path negotiates HTTP/2 by runtime default; explicit disable is not supported.',
    );
  }
