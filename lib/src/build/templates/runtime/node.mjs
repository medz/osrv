...
function readRequestedHttp2(options = {}, env = process.env) {
  return parseBoolean(
    options.http2 ?? env.OSRV_NODE_HTTP2 ?? env.OSRV_HTTP2,
  );
}
...
const presetRequestedHttp2 = readRequestedHttp2({}, process.env);
setRuntimeCapabilities({
  http2:
    presetProtocol === 'https' &&
    hasTlsFromEnvironment() &&
    (presetRequestedHttp2 ?? true),
  websocket: true,
});
...
export async function serve(options = {}) {
  ...
  const requestedHttp2 = readRequestedHttp2(options, process.env);
  if (requestedHttp2 === true && protocol !== 'https') {
    console.warn(
      `${runtimeConfig.logPrefix} HTTP/2 requested without HTTPS/TLS, falling back to HTTP/1.1.`,
    );
  }

  if (protocol === 'https' && (requestedHttp2 ?? true)) {
