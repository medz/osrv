...
void main() {
  ...
  test(
    'node runtime exposes websocket handling through the current osrv surface',
    () async {
      final chatSawWebSocketCapability = Completer<bool>();
      final chatSawUpgradeRequest = Completer<bool>();
      final chatSawWebSocketObject = Completer<bool>();
      ...
      final runtime = await serve(
        Server(
          fetch: (request, context) {
            ...
            if (uri.path == '/chat') {
              if (!chatSawWebSocketCapability.isCompleted) {
                chatSawWebSocketCapability.complete(
                  context.capabilities.websocket,
                );
              }
              if (!chatSawWebSocketObject.isCompleted) {
                chatSawWebSocketObject.complete(webSocket != null);
              }
              if (!chatSawUpgradeRequest.isCompleted) {
                chatSawUpgradeRequest.complete(
                  webSocket?.isUpgradeRequest ?? false,
                );
              }
      ...
      final httpResponse = await _fetchText(runtime.url!.resolve('/plain'));
      expect(httpResponse.status, 200);
      expect(httpResponse.text, '{"websocket":true,"upgrade":false}');

      final webSocket = await ws.WebSocket.connect(
        Uri(
          scheme: 'ws',
          host: runtime.url!.host,
      ...
      final events = StreamIterator<ws.WebSocketEvent>(webSocket.events);
      ...
      expect(
        events.current,
        isA<ws.TextDataReceived>().having(
          (event) => event.text,
          'text',
          'connected',
        ),
      ...
      expect(await chatSawWebSocketCapability.future, isTrue);
      expect(await chatSawWebSocketObject.future, isTrue);
      expect(await chatSawUpgradeRequest.future, isTrue);
    },
  );
  ...
  test('node runtime.close waits for active websocket sessions', () async {
    ...
    await expectLater(
      sessionClosed.future.timeout(const Duration(seconds: 5)),
      completes,
    );
    await expectLater(
    ...
    try {
      ...
      // The runtime may already have completed the shutdown handshake.
    }
  });
